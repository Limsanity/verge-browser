...
更多 API 详见 [`docs/api.md`](./docs/api.md)。
...
1. 继续强化运行时容器，直到 Chromium、CDP 与桌面接入稳定可靠。
2. 将 Playwright 与 CDP 兼容性验证扩展到低级冒烟检查之外。
3. 加强 session 管理与 WebSocket 生命周期行为。
4. 扩展文件与集成测试覆盖。
5. 为浏览器重启和运行时降级增加故障注入测试。
6. 增加部署打磨、可观测性和生产加固。
