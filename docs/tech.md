...
- `GET /sandbox/{sandbox_id}/vnc/` -> noVNC 静态资源
- `WS /sandbox/{sandbox_id}/vnc/websockify` -> WebSocket 代理

### 9.4 VNC 鉴权

VNC 浏览器页面无法稳定带 Authorization Header，因此推荐短时票据：

- 主 API 通过 `POST /sandbox/{sandbox_id}/vnc/apply` 签发 ticket，并直接返回带签名的 `vnc_url`
...
```text
/sandbox/{sandbox_id}/vnc/?ticket=xxxxx
```
...
- 对外暴露的 `cdp_url` 应是平台稳定 URL 加短时票据，例如 `/sandbox/{id}/cdp/browser?ticket=...`。
...
- 确保二进制消息、文本消息都能透传
- 不要求调用方额外设置 Authorization header；鉴权通过 URL 中的短时 ticket 完成
...
当前实现不再单独提供 `GET /browser/viewport`。

这些字段统一聚合到 `GET /sandbox/{id}` 的 `browser` 对象中：
...
- `POST /sandbox/{id}/browser/screenshot`
...
- `GET /sandbox/{id}/files/list?path=/workspace`
- `GET /sandbox/{id}/files/read?path=/workspace/a.txt`
- `POST /sandbox/{id}/files/write`
- `POST /sandbox/{id}/files/upload`
- `GET /sandbox/{id}/files/download?path=/workspace/report.pdf`
- `DELETE /sandbox/{id}/files?path=/workspace/a.txt`
...
- `POST /sandbox/{id}/browser/reconnect-hint`
...
`POST /sandbox/{id}/browser/restart`
...
- `POST /sandbox/{id}/resume`
- `DELETE /sandbox/{id}`

#### Browser

- `POST /sandbox/{id}/browser/screenshot`
- `POST /sandbox/{id}/browser/actions`
- `POST /sandbox/{id}/browser/restart`
- `POST /sandbox/{id}/cdp/apply`
- `WS /sandbox/{id}/cdp/browser`
- `GET /sandbox/{id}` 中聚合 `browser/info` 与 `browser/viewport` 信息

#### VNC

- `POST /sandbox/{id}/vnc/apply`
- 请求体支持 `mode` 和 `ttl_sec`
- 响应返回 `ticket`、`vnc_url`、`mode`、`ttl_sec`、`expires_at`
- `GET /sandbox/{id}/vnc/`
- `WS /sandbox/{id}/vnc/websockify`

#### Files

- `GET /sandbox/{id}/files/list`
- `GET /sandbox/{id}/files/read`
- `POST /sandbox/{id}/files/write`
- `POST /sandbox/{id}/files/upload`
- `GET /sandbox/{id}/files/download`
...
1. 实现 `GET /sandbox/{id}` 中的 browser 聚合信息
...
1. 实现 `/sandbox/{id}/cdp/apply`
2. 实现 `/cdp/browser` WebSocket 代理
3. 加入 ping/pong 保活
...
- 创建与详情接口不直接返回带 ticket 的 CDP/VNC 链接，因为它们具备短时效性，不适合缓存。
- 当前契约是通过 `POST /sandbox/{id}/cdp/apply` 与 `POST /sandbox/{id}/vnc/apply` 按需申请票据。

### 21.2 CDP WebSocket 代理

```python
@app.websocket("/sandbox/{sandbox_id}/cdp/browser")
